...
def build_parser():
    ...
    # 模型
    parser.add_argument('--diffusion-config', type=str, default='./config/base_solospeech_tse_cfm_urgent2026_MoLExLoRA_lb.yaml')
    ...
    parser.add_argument('--molex-lb-coef', type=float, default=0.01,
...
def main():
    ...
    TARGET_HOURS = 12.0             # select 12-hour data to finetune
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_loader)):
            ...
            if args.use_molex and args.molex_lb_type != "none" and args.molex_lb_coef > 0.0:
                ...
                loss = loss + args.molex_lb_coef * lb_loss_total
            ...
            if not torch.isnan(loss):
                ...
                if accelerator.is_main_process and global_step % args.log_step == 0:
                    with open(os.path.join(args.log_dir, 'log.txt'), 'a') as f:
                        ...
                        f.write(
                            'Epoch: [{}][{}]    Batch: [{}][{}]    Loss: {:.6f}    Main: {:.6f}    LB: {:.6f}\n'.format(
                                epoch + 1,
                                args.epochs,
                                step + 1,
                                len(train_loader),
