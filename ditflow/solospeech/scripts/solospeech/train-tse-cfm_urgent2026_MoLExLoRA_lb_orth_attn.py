...
class MoLExLinear(nn.Module):
    ...
    额外：
    - 支持 per-layer load-balancing loss（Switch / CV 风格）：
      在 forward 中计算并保存在 self._last_lb_loss 里，
      通过 get_lb_loss() 暴露给外部。
    """
    ...
    def __init__(
        ...
        noise_std: float = 1.0,
        bias: bool = True,
        lb_loss_type: str = "switch",  # 'none' | 'switch' | 'cv'
    ):
        super().__init__()
        ...
        self.lb_loss_type = lb_loss_type  # 决定 LB loss 类型
        self._last_lb_loss = None         # 存一层上一次 forward 的 LB loss
    ...
    def reset_parameters(self):
        ...
        # 关键：A 随机，B 置零，确保 ΔW 初始≈0 但梯度不为 0
        nn.init.kaiming_uniform_(self.lora_A, a=5 ** 0.5)
        nn.init.zeros_(self.lora_B)

        # gating network：保持 0 即可，配合噪声
    ...
    def _compute_load_balancing_loss(self, probs: torch.Tensor,
                                     topk_indices: torch.Tensor) -> torch.Tensor:
        """
        根据当前 batch 的 gating 统计计算 per-layer LB loss。

        probs: [N, E]，对 noisy logits 做 softmax 得到的概率分布 p_{t,e}
        topk_indices: [N, k]，每个 token 选中的 k 个专家 index
        """
        if self.lb_loss_type == "none":
        ...
        # importance: P_e = mean_t p_{t,e}
        importance = probs.mean(dim=0)  # [E]

        # load: f_e = mean_t 1[e in top-k]
        mask = torch.zeros_like(probs)  # [N, E]
        mask.scatter_(1, topk_indices, 1.0)
        load = mask.mean(dim=0)        # [E]

        if self.lb_loss_type == "switch":
            # Switch-style: E * sum_e f_e * P_e
            lb = self.num_experts * (importance * load).sum()
        elif self.lb_loss_type == "cv":
            eps = 1e-6

            # CV^2 for importance
            imp_mean = importance.mean()
            imp_var = ((importance - imp_mean) ** 2).mean()
            cv_imp2 = imp_var / (imp_mean ** 2 + eps)

            # CV^2 for load
            load_mean = load.mean()
            load_var = ((load - load_mean) ** 2).mean()
            cv_load2 = load_var / (load_mean ** 2 + eps)
    ...
    def get_lb_loss(self) -> torch.Tensor:
        """
        返回该层最近一次 forward 计算到的 LB loss（若无则 0）。
        """
        if self._last_lb_loss is None:
            return torch.tensor(0.0, device=self.weight.device)
        return self._last_lb_loss
    
    def get_orth_loss(self) -> torch.Tensor:
        """
        返回该层最近一次 forward 计算到的 orth loss（若无则 0）。
        """
    ...
    def _compute_orth_loss(self) -> torch.Tensor:
        ...
        # LoRA experts: A: [E, r, in], B: [E, out, r]
        if self.lora_A.ndim != 3 or self.lora_B.ndim != 3 or self.lora_A.shape[0] != self.lora_B.shape[0]:
            return torch.tensor(0.0, device=device, dtype=dtype)

        E = self.lora_A.shape[0]
        if E <= 1:
            return torch.tensor(0.0, device=device, dtype=dtype)

        # W: [E, out, in]
        W = torch.einsum("eor,eri->eoi", self.lora_B, self.lora_A)

        Wn = torch.nn.functional.normalize(W, p=2, dim=-1, eps=1e-12)  # [E, O, I]

        # Wn: [E, O, I]
        # G[e, o, p] = sum_i Wn[e, o, i] * Wn[e, p, i]  ->  [E, O, O]
        G = torch.einsum('eoi,epi->eop', Wn, Wn)

        # 只惩罚非对角
        G2 = G.pow(2)
        G2 = G2 - torch.diag_embed(torch.diagonal(G2, dim1=1, dim2=2))
        loss = G2.sum(dim=(1, 2)).mean() / (G.shape[1] * (G.shape[1] - 1))

        # _, O, _ = G.shape
        # I = torch.eye(O, device=G.device, dtype=G.dtype).expand(E, O, O)

        # loss = ((G - I) ** 2).sum(dim=(1, 2)).mean()
    ...
    def forward(self, x):
        ...
        # full softmax 概率（用于 importance 统计）
        ...
        # 5) LB loss（per-layer）
        if self.training:
            self._last_lb_loss = self._compute_load_balancing_loss(probs, topk_indices)
...
class MoLExFeedForward(nn.Module):
    ...
    *注意：目前主路径是通过 apply_molex 把已有 FeedForward.w1/w2 换成 MoLExLinear。
    """
...
def apply_molex(
    ...
    k: int = 2,
    noise_std: float = 1.0,
    target_keywords: Iterable[str] = ("mlp.w1",
    "mlp.w2",
    "attn.to_q",
    "attn.to_k",
    "attn.to_v",
...
def build_parser():
    ...
    # 模型
    parser.add_argument('--diffusion-config', type=str, default='./config/base_solospeech_tse_cfm_urgent2026_MoLExLoRA_lb_orth_attn.yaml')
    ...
    parser.add_argument('--molex-targets', type=str,
                        default='mlp.w1,mlp.w2,attn.to_q,attn.to_k,attn.to_v,attn.proj',
    ...
    # LB loss 相关
    parser.add_argument('--molex-lb-type', type=str, default='switch',
                        choices=['none', 'switch', 'cv'],
                        help='Type of load-balancing loss for MoLEx experts: none | switch | cv')
    parser.add_argument('--molex-lb-coef', type=float, default=0.001,
                        help='Global coefficient for MoLEx load-balancing loss')
    parser.add_argument('--molex-orth-coef', type=float, default=1.0,
                        help='Global coefficient for MoLEx orth loss')
...
def main():
    ...
    TARGET_HOURS = 30.0             # select the number of hours for data to finetune
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_loader)):
            ...
            # 原任务 loss
            main_loss = masked_mse(pred, velocity if args.v_prediction else noise, pred_mask)
            loss = main_loss

            # ===== MoLEx load-balancing loss =====
            lb_loss_total = torch.tensor(0.0, device=accelerator.device)
            orth_loss_total = torch.tensor(0.0, device=accelerator.device)
            if args.use_molex and args.molex_lb_type != "none" and args.molex_lb_coef > 0.0:
                for m in model.modules():
                    if isinstance(m, MoLExLinear):
                        lb_loss_total = lb_loss_total + m.get_lb_loss()
                        orth_loss_total = orth_loss_total + m.get_orth_loss()

                loss = loss + args.molex_lb_coef * lb_loss_total + args.molex_orth_coef * orth_loss_total 
            ...
            if not torch.isnan(loss):
                ...
                if accelerator.is_main_process and global_step % args.log_step == 0:
                    with open(os.path.join(args.log_dir, 'log.txt'), 'a') as f:
                        ...
                        f.write(
                            'Epoch: [{}][{}]    Batch: [{}][{}]    Loss: {:.6f}    Main: {:.6f}    LB: {:.6f}   Orth: {:.6f}\n'.format(
                                epoch + 1,
                                args.epochs,
                                step + 1,
                                len(train_loader),
                                losses / args.log_step,
                                main_loss.item(),
                                lb_loss_total.item(),
                                orth_loss_total.item()
                            )
                        )
                    losses = 0.0
            else:
        ...
        if accelerator.is_main_process and ((epoch + 1) % args.save_every == 0):
            ...
            save_obj = {
                ...
                "molex_lb_type": args.molex_lb_type,
                "molex_lb_coef": args.molex_lb_coef,
            }
